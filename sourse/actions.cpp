...
// возвращает их скалярное произведение в виде числа с плавающей запятой
float dot_product(COORD a, COORD b) {
    float result = a.x * b.x + a.y * b.y;
    return result;
}

//Return distance between two points A(x,y) & B(x,y) (по теореме пифагора)
...
// вычисляет ближайшую точку на отрезке (a, b) до заданной точки p, а затем проверяет, находится ли эта точка в пределах отрезка (a, b). 
// Если нет, то она возвращает ближайшую из точек a и b.
COORD findClosestPointLineSegment(COORD a, COORD b, COORD p) {
    ...
    // Вычисление переменной d, которая представляет квадрат суммы квадратов координат вектора A->B.
    ...
    // вычисляем координаты ближайшей точки используя формулу для линейной интерполяции.
    else {
...
// позволяет определить положение глаза рыбы относительно её глобального центра, используя внутренние координаты глаза, угол, ширину и высоту.
COORD getCapybaraEyeCoord(COORD global_center, COORD inner_eye_coord, float angle, float width, float height) {
...
// возвращает координаты ближайшей точки относительно fish_eye.
...
float angle_between_two_coordinates(COORD coord1, COORD coord2) {
    ...
    vect2.i = delta_x;
    vect2.j = 0;
    angle = ((acos((vect1.i * vect2.i + vect1.j * vect2.j) / ((sqrtf(vect1.i * vect1.i + vect1.j * vect1.j)) * (sqrtf(vect2.i * vect2.i + vect2.j * vect2.j)))) * 180 / 3.14159265358979323846));
